...
class MenuDialog(context: Context) {
    ...
    private lateinit var listener: MenuDialogOKClickedListener
   ...
   // 대화 상자 레이아웃의 View ID 변수 생성
    ...
    // 메뉴 ID 변수
    private lateinit var menuCate:String
    ...
    fun start(position: Int) {
        ...
        dlg.setContentView(R.layout.dailog)
...
//        dlg.setCancelable(false)
        ...
        when (position) {
            ...
            0 -> {
                ...
                val coffeeDetail = mutableListOf("아침 잠을 깨워주는", "점심을 마무리하는", "오늘 고생한 나를 위한")
                menuCate = "1" + promotionGoods(coffeeImg, coffeeName, coffeeDetail)
                menuId = menuCate.toInt()
            }
            ...
            3 -> {
                ...
                menuCate = "2" + promotionGoods(coffeeImg, coffeeName, coffeeDetail)
                menuId = menuCate.toInt()
            }
            ...
            4 -> {
                ...
                menuCate = "3" + promotionGoods(coffeeImg, coffeeName, coffeeDetail)
                menuId = menuCate.toInt()
            }

            // 판촉 상품을 미처 준비하지 않았거나 고려하지 않은 경우
            else -> {
                menuId = 2
            }
        }
    ...
    fun setOnOKClickedListener(listener: (Int) -> Unit) {
        this.listener = object : MenuDialogOKClickedListener {
            override fun onOKClicked(fragmentState: Int) {
                listener(fragmentState)
            }
        }
    }

    interface MenuDialogOKClickedListener {
        fun onOKClicked(fragmentState: Int)
    }

    // 각 시간대에 맞는 판촉 상품 나타내기
    // LocalTime, LocalDate -> @RequiresApi(Build.VERSION_CODES.N), @SuppressLint("NewApi"): 메서드
    @SuppressLint("NewApi")
    @RequiresApi(Build.VERSION_CODES.N)
    private fun promotionGoods(Img: MutableList<Int>, name: MutableList<String>, detail: MutableList<String>): String {
